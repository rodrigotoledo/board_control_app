    ...
    <SafeAreaView className='bg-gray-800 h-full'>
      ...
      <ScrollView className="container my-2 bg-gray-900">
        <View className="mx-4 my-10">
          {!isLoadingProjects &&
            projects?.map((project, index) => <ProjectItem key={project.id} project={project} index={index} completeProject={completeProject} />)}
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
const ProjectItem = ({ project, index, completeProject }) => {
